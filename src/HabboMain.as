package
{
   ...
   public class HabboMain extends Sprite
   {
      ...
      private function updateLoadingBar(param1:DisplayObjectContainer, param2:Number) : void
      {
         ...
         var _loc13_:int = var_29.getNumberOfFilesPending() + var_29.getNumberOfFilesLoaded();
         var _loc14_:Number = 1 - const_27 + var_29.getNumberOfFilesLoaded() / _loc13_ * const_27;
         _loc8_.x = 1 + 1;
         _loc8_.y = 1 + 1;
      ...
      private function configLoadedHandler(param1:AssetLoaderEvent) : void
      {
         ...
         {
            Habbo.reportCrash("Failed to download external configuration document " + _loc2_.assetLoader.url + "!",Core.const_173,null);
         }
      }
      ...
      private function initializeCore() : void
      {
         ...
         catch(error:Error)
         {
            ...
            Core.crash("Failed to initialize the core: " + error.message,Core.const_1009,error);
         }
      }
      ...
      protected function onAddedToStage(param1:Event = null) : void
      {
         ...
         {
            Habbo.trackLoginStep(Habbo.const_29);
            Habbo.reportCrash("Failed to prepare the core: " + error.message,Core.const_1009,error);
            Core.dispose();
         }
      }
      ...
      {
         var _loc2_:XmlAsset = var_29.assets.getAssetByName(param1.assetName) as XmlAsset;
         if(!_loc2_ || !_loc2_.content)
         {
            Habbo.reportCrash("Download external configuration document is null!",Core.const_173,null);
         }
         if(var_29)
         {
            var_29.readConfigDocument(XML(_loc2_.content),var_1);
         }
         else
         {
            Habbo.reportCrash("Core vanished while downloading config document!",Core.const_173,null);
         }
      }
